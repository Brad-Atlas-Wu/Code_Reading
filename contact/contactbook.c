...
/*=>codes above predefine strings reside for using . & initial output interface   */

//=> count the the number of members & return 
...
//=>get user's choice and execute
//1: next page
//2:
void ListWaitUser(int iCurpage)
{
	int iKey;
	int iPkey=0;//=>P refers to Previous,N refers to Next
	int iNkey=0;
	int iDataTotalPage;
	int iDataTotalCount;
	...
	if(iCurpage==1 && iDataTotalPage>1)//=>at the first page,can only go next
	{
		printf("%s\n",PageInductText[0]);
		iNkey=1;iPkey=0;
	}
	else if(iCurpage>1 && iCurpage<iDataTotalPage)//=>pages between, can go back and forth
	{
		printf("%s\n",PageInductText[1]);
		iNkey=1;iPkey=1;
	}
	else if(iCurpage==iDataTotalPage && iCurpage>1)//=>last page ,can only go to previous pages
	{
		printf("%s\n",PageInductText[2]);
		iNkey=0;iPkey=1;
	}
	else//=>no data or only one page
	{
		printf("%s\n",PageInductText[3]);
	}
	scanf("%d",&iKey);
	if(iKey==2 && iPkey==1)
		ViewList(--iCurpage);//=>recrusion,till key is not 1 or 2
	if(iKey==1&& iNkey==1)
		ViewList(++iCurpage);
}
...
//=>get&check additonal input
void InputData(struct contacts* pcontacts)
{
	int iInputCount=200;	//count the times of (error)input
	printf("%s\n",InputTip[0]);
...
//=> add new members to contact
void Addcontacts()
{
	struct contacts contacts;
	int bContinue=1;//=>judge if continue input new members
	int iSel;//=>get "1" or "2" choice
	memset(&contacts,0,sizeof(contacts));	//=>initiate to 0
	InputData(&contacts); 	//=>get additional input,store in struct contacts temporily
	SavecontactsToFile(&contacts);//=>save it to files
	while(bContinue)
	{
		...
		{
			ClearScreen();
			memset(&contacts,0,sizeof(contacts));//=>initiate
			InputData(&contacts);
...
void SearchNumber()
{
	...
	int iDataTotalCount;
	struct contacts contacts;
	int iPos;//=>keep eyes on position
	int i;
	printf("%s\n",InputTelTip);
	scanf("%s",cTelNumber);
	iDataTotalCount=GetFileDataCount();//=>know when to stop reading
	file=fopen("contacts.dat","r");
	for(i=0;i<iDataTotalCount;i++)
	{
		printf("%d",iDataTotalCount);
		iPos=sizeof(struct contacts)*i;
		fseek(file,iPos,SEEK_SET);//=>locate the start-reading point
...
//=>list all members by page
void ViewList(int iCurpage)
{
	FILE *file;
	int i;
	struct contacts contacts;	
	int iDataTotalPage;	//=>count how many pages we need
	int iDataTotalCount;	//=>how many contacts in total
	int iDataCountPerPage;	//=>how many contacts should be printed on the current page
	int iPos;	//=>  locate the position in the file
	int j;
	ClearScreen();
	iDataCountPerPage=20;	//=>assume that 20 contacts a page
	iDataTotalCount=GetFileDataCount();	//get to know how mant contacts in the file in total
	
	if(iDataTotalCount==0)	//=>no data,so need only one page
		iDataTotalPage=0;
	else	//=>else,get the number of pages that is needed
		iDataTotalPage=iDataTotalCount/20+1;
	
	if(iCurpage==iDataTotalPage)	//=>if it's the last pages,then count how many contaces left for the last pages
	...
	if(iDataTotalPage>0)
	{
		if(iCurpage>0 && iCurpage<=iDataTotalPage)
		{
			...
			{
				memset(&contacts,0,sizeof(struct contacts));	//=>initialise structure

				/*locate & read structure*/
				iPos=sizeof(struct contacts)*((iCurpage-1)*20+i);
				fseek(file,iPos,SEEK_SET);
				fread(&contacts,sizeof(struct contacts),1,file);
				/*show list*/
				printf("%d   ",(iCurpage-1)*20+(i+1));	//show the index
	...
	else	//no data to list
	{
		printf("Press Enter return...");
		getch();

	}
		if(file!=NULL)	//it's wired to check at the end before close??!
		fclose(file);
}
...
//=>all move backward for 0ne structure,that remove a structure
void DeleteData()
{
	FILE *file;
	int iIndex;
...
//
//react according to selection from users
void mainloop()
{
	ClearScreen();
	ShowWelcome();
